...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    // ── Recipe Magic (NVIDIA Llama 3.2 11B Vision) ──
    if (pathname === '/api/recipe-magic' && req.method === 'POST') {
      ...
      if (!nvKey) { send(res, 200, { ok: false, error: 'NVIDIA_API_KEY not set' }); return; }

      const payload = JSON.stringify({
        messages: [{
          role: 'user',
          content: [
            { type: 'text', text: 'Extract recipe information from this food image. Return ONLY valid JSON with these fields: title, subtitle, description, prep, cook, servings, ingredients (array of objects with group and items array), instructions (array of strings), equipment (array), tips, video. If ingredients have groups, separate them. If no info for a field, use empty string or empty array. Return ONLY the JSON object, no markdown, no code blocks, no explanation.' },
      ...
      try {
        const resp = await new Promise((resolve, reject) => {
          const r = https.request({
            ...
            method: 'POST',
            headers: { 'Authorization': 'Bearer ' + nvKey, 'Content-Type': 'application/json', 'Content-Length': Buffer.byteLength(payload) },
          }, (res2) => {
        ...
        if (resp.status !== 200) { send(res, 200, { ok: false, error: 'NVIDIA error: ' + resp.body.slice(0, 300) }); return; }
